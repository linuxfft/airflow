...
async def training_server_update_templates():
    ...
    for url in list(dict.fromkeys([analysisUrl,
                                   trainingUrl])):  # remove duplicate
        async with RetryClient(timeout=ClientTimeout(300)) as client:
            async with client.post(headers=headers, url=url,
                                   retry_attempts=5) as r:
                r.raise_for_status()
                resp = await r.read()
                _logger.debug("cas/templates called, url: {} resp: {}".format(
                    url, resp))
