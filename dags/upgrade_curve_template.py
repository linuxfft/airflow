...
def template_upgrade_handler(ch, method: pika.spec.Basic.Deliver,
    ...
    try:
        ...
        raise e
    try:
        key, val = CurveTemplateModel.get_fuzzy_active(key=template_name,
        ...
        CurveTemplateModel.set(
            key=key, value=template,
            serialize_json=True)  # 此业务场景下 params不会变化故覆盖现有的variable
...
def curve_template_upgrade_task(**kwargs):
    ...
    _logger.debug("{} context: {}".format('curve_template_upgrade_task',
                                          kwargs))
    mq_connection = ClsResultMQ(**ClsResultMQ.get_result_mq_args(
        key='qcos_rabbitmq'))
