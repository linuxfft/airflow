...
def timeout_count(period, threshold, session=None):
    ...
    if delta:
        return tasks.filter(TaskInstance.execution_date > (timezone.utcnow() +
                                                           delta)).first()[0]
    return 0
...
def report_analysis_timeout(**kwargs):
    period = Variable.get('report_analysis_timeout_period', {'hours': -1},
                          deserialize_json=True)
    threshold = Variable.get('report_analysis_timeout_threshold', 10)
    count = timeout_count(period, threshold)
    if count == 0:
        _logger.info(u'未达到报警条件, period: {}, threshold: {}'.format(
            period, threshold))
        return
    mails_analysis_timeout = Variable.get(mail_key, [])
    if not mails_analysis_timeout or not len(mails_analysis_timeout) > 0:
        raise Exception(u'分析超时报警邮箱未配置:{}，请在变量中添加"{}"变量'.format(
            str(mails_analysis_timeout), mail_key))
    send_email(to=mails_analysis_timeout,
               subject=u'分析任务超时报警',
               html_content=report_message(period, count, threshold),
               mime_charset='utf-8')


dag = DAG('report_analysis_timeout',
          description=u'分析任务超时报警',
