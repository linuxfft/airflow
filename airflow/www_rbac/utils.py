...
def should_hide_value_for_key(key_name):
    ...
    if key_name:
        ...
        field_comp = any(s in key_name.lower()
                         for s in DEFAULT_SENSITIVE_VARIABLE_FIELDS)
...
def json_response(obj):
    ...
    """
    return Response(response=json.dumps(obj, indent=4, cls=AirflowJsonEncoder),
                    status=200,
...
def datetime_f(attr_name):
    def dt(attr):
        ...
        return Markup(
            '<nobr><time title="" datetime="{}">{}</time></nobr>').format(
                as_iso, f)
...
def get_attr_renderer():
    attr_renderer = {
        'bash_command':
        lambda x: render(x, lexers.BashLexer),
        'hql':
        lambda x: render(x, lexers.SqlLexer),
        'sql':
        lambda x: render(x, lexers.SqlLexer),
        'doc':
        lambda x: render(x, lexers.TextLexer),
        'doc_json':
        lambda x: render(x, lexers.JsonLexer),
        'doc_rst':
        lambda x: render(x, lexers.RstLexer),
        'doc_yaml':
        lambda x: render(x, lexers.YamlLexer),
        'doc_md':
        wrapped_markdown,
        'python_callable':
        lambda x: render(get_python_source(x), lexers.PythonLexer),
    }
...
class FilterErrorTagReg(fab_sqlafilters.BaseFilter):
    ...
    def apply(self, query, value):
        query, field = fab_sqlafilters.get_field_setup_query(
...
class UtcAwareFilterConverter(fab_sqlafilters.SQLAFilterConverter):
    conversion_table = ((('is_errortag', [FilterErrorTagReg]), ) +
                        (('is_utcdatetime', [
                            UtcAwareFilterEqual, UtcAwareFilterGreater,
                            UtcAwareFilterSmaller, UtcAwareFilterNotEqual
                        ]), ) +
